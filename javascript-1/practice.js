...
function bigOrSmall (arr){
	let answers= []; //this is a bucket. It starts off empty - we need to fill it and then return the bucket.
	...
	//Attempt 3 - simple (only use classic for loops if you need the index!)
	arr.forEach(num => {
		if (num>100){
			answers.push('big');
		}
...
function arrayReverser (arr){
	...
	// for (let i = 0; i < arr.length; i++){
	// 	let revIndex = arr.length - i - 1; 
	// 	reversed.push(arr[revIndex]);
	// }
	return reversed;
}
//////////////////PROBLEM 14 - 18 Setup////////////////////
...
// let forTheLoveOfGeorge= notGeorge.map (notGeorge.slice (0,5) => 'George');
...
// Code Here
let enemies= people.filter(people.friend === true);
